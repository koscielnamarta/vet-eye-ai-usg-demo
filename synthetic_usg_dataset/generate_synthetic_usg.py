...
# ------------------------------------------------------------------
# HARD HOLD-OUT: ten sam problem, ale trudniejsza dystrybucja obrazu
# ------------------------------------------------------------------
def apply_hard_usg_shift(img):
    """
    Naklada dodatkowe artefakty akwizycji na juz wygenerowany syntetyczny obraz.

    To NIE tworzy nowej klasy ani nie zmienia etykiety. Celem jest symulacja
    trudniejszego holdoutu:
    - gorsze kadrowanie / przesuniecie sondy,
    - wiekszy szum speckle i elektroniki,
    - zmienna jasnosc / kontrast / gamma,
    - czesciowe cienie akustyczne,
    - slabsza rozdzielczosc / rozmycie.

    Wazne: model nie powinien trenowac na tym zbiorze. To stress-test generalizacji
    poza "czysta" dystrybucja generatora.
    """
    img = img.convert("L")

    # 1. Delikatna rotacja i translacja - symulacja innego ustawienia glowicy.
    angle = random.uniform(-10, 10)
    tx = random.randint(-18, 18)
    ty = random.randint(-12, 16)
    ...
    # 2. Losowy crop/zoom - czesc anatomii moze byc blizej krawedzi.
    if random.random() < 0.80:
        zoom = random.uniform(0.78, 0.96)
        crop_w = int(IMG_SIZE * zoom)
        crop_h = int(IMG_SIZE * zoom)
        left = random.randint(0, IMG_SIZE - crop_w)
        top = random.randint(0, IMG_SIZE - crop_h)
        img = img.crop((left, top, left + crop_w, top + crop_h))
        img = img.resize((IMG_SIZE, IMG_SIZE), Image.Resampling.BILINEAR)

    # 3. Gorsza rozdzielczosc - downsample + upsample.
    if random.random() < 0.65:
        small = random.randint(150, 220)
        img = img.resize((small, small), Image.Resampling.BILINEAR)
        img = img.resize((IMG_SIZE, IMG_SIZE), Image.Resampling.BILINEAR)

    # 4. Zmienny kontrast i jasnosc.
    img = ImageEnhance.Contrast(img).enhance(random.uniform(0.65, 1.45))
    img = ImageEnhance.Brightness(img).enhance(random.uniform(0.70, 1.25))

    arr = np.array(img).astype(np.float32)

    # 5. Depth attenuation / TGC mismatch - dol obrazu bywa slabszy.
    yy, xx = np.mgrid[0:IMG_SIZE, 0:IMG_SIZE]
    depth_gain = np.linspace(random.uniform(0.95, 1.15),
                             random.uniform(0.45, 0.90),
                             IMG_SIZE).reshape(-1, 1)
    lateral_gain = np.linspace(random.uniform(0.75, 1.15),
                               random.uniform(0.75, 1.15),
                               IMG_SIZE).reshape(1, -1)
    arr = arr * depth_gain * lateral_gain

    # 6. Mocniejszy speckle i szum elektroniki.
    speckle = np.random.rayleigh(scale=random.uniform(0.55, 1.15), size=arr.shape)
    speckle = speckle / np.mean(speckle)
    speckle_weight = random.uniform(0.35, 0.85)
    arr = arr * ((1 - speckle_weight) + speckle_weight * speckle)
    arr += np.random.normal(0, random.uniform(5, 16), arr.shape)

    # 7. Gamma shift - inna charakterystyka wyswietlania/akwizycji.
    arr = np.clip(arr, 0, 255)
    gamma = random.uniform(0.65, 1.45)
    arr = 255 * np.power(arr / 255, gamma)

    # 8. Dodatkowe cienie akustyczne / dropouty.
    if random.random() < 0.75:
        x_center = random.randint(50, 205)
        half_width = random.randint(10, 35)
        y_start = random.randint(60, 145)
        factor = random.uniform(0.25, 0.65)
        shadow = (np.abs(xx - x_center) <= half_width) & (yy > y_start)
        arr[shadow] *= factor

    if random.random() < 0.40:
        # szeroki, ukośny pas slabszego sygnalu
        slope = random.uniform(-0.35, 0.35)
        x0 = random.randint(30, 210)
        width = random.randint(12, 28)
        band_center = x0 + slope * (yy - IMG_SIZE / 2)
        band = np.abs(xx - band_center) < width
        arr[band] *= random.uniform(0.35, 0.70)

    # 9. Finalne rozmycie - gorszy fokus.
    img = Image.fromarray(np.clip(arr, 0, 255).astype(np.uint8))
    if random.random() < 0.75:
        img = img.filter(ImageFilter.GaussianBlur(radius=random.uniform(0.7, 2.7)))

    return img


def generate_image_hard(class_name):
    """Tworzy obraz danej klasy z dodatkowym przesunieciem dystrybucji."""
    clean_img = generate_image(class_name)
    return apply_hard_usg_shift(clean_img)
...
def generate_hard_holdout(seed=777, n_per_class=200, output_subdir="synthetic_hard_holdout"):
    """
    Generuje trudniejszy holdout syntetyczny z przesunieciem dystrybucji.

    Roznica wzgledem --holdout:
    - --holdout: ten sam generator i ta sama dystrybucja, tylko inny seed,
    - --hard-holdout: ten sam problem klasyfikacyjny, ale dodatkowe artefakty,
      wiekszy szum, gorsze kadrowanie, zmienny kontrast i cienie.

    Output: dataset/synthetic_hard_holdout/{CLASS}/CLASS_hardNNNN.png
    """
    random.seed(seed)
    np.random.seed(seed)
    print(f"[HARD_HOLDOUT] seed={seed}, n_per_class={n_per_class}, subdir={output_subdir}")
    ...
    for class_name in CLASSES:
        cls_dir = out_root / class_name
        cls_dir.mkdir(parents=True, exist_ok=True)
        print(f"[HARD_HOLDOUT/{class_name}] target: {n_per_class} obrazow")

        generated, skipped, failed = 0, 0, 0
        for i in range(n_per_class):
            fname = f"{class_name}_hard{i:04d}.png"
            out_path = cls_dir / fname
            ...
            img = generate_image_hard(class_name)
            ok = save_image_safely(img, out_path, class_name=class_name)
            if ok:
                generated += 1
            else:
        ...
        print(f"[HARD_HOLDOUT/{class_name}] DONE: wygenerowano {generated}, "
              f"pominieto {skipped}, BLEDY: {failed}")
        total_generated += generated
        total_skipped += skipped
        total_failed += failed

    print(f"\n[HARD_HOLDOUT] Lacznie: wygenerowano {total_generated}, "
          f"pominieto {total_skipped}, BLEDY: {total_failed}")
    print(f"[HARD_HOLDOUT] Folder: {out_root}")
    return total_failed


def main(only_class=None, write_csv=False, verify=False, repair=False,
         holdout=False, holdout_seed=123, holdout_n=200,
         hard_holdout=False, hard_holdout_seed=777, hard_holdout_n=200):
    ...
    if holdout:
        ...
        return
    if hard_holdout:
        generate_hard_holdout(seed=hard_holdout_seed, n_per_class=hard_holdout_n)
        return
    if write_csv:
        write_split_csv()
        return
...
if __name__ == "__main__":
    ...
    if "--verify" in args:
        ...
        if "--csv" in args:
            main(write_csv=True)
    elif "--hard-holdout" in args:
        # python generate_synthetic_usg.py --hard-holdout
        # opcjonalnie: --hard-holdout-seed 777 --hard-holdout-n 200
        seed = 777
        n = 200
        if "--hard-holdout-seed" in args:
            seed = int(args[args.index("--hard-holdout-seed") + 1])
        if "--hard-holdout-n" in args:
            n = int(args[args.index("--hard-holdout-n") + 1])
        main(hard_holdout=True, hard_holdout_seed=seed, hard_holdout_n=n)
